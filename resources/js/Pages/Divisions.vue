                ...
                <v-row>
                 <v-col class="mb-2">
                   <v-btn color="primary" size="small" rounded="lg" prepend-icon="mdi-plus-thick" @click="openDialog()" class="mr-2 mb-2">Add new</v-btn>
                     <v-btn color="#9CCC65"   size="small" rounded="lg" prepend-icon="mdi-refresh" @click="$inertia.visit('/divisions')" class="mr-2 mb-2">Refresh</v-btn>
                 </v-col>
                </v-row>
                        ...
                        <v-card-text class="mt-3">
                             <v-row>
                                <v-col cols="12" md="12" sm="12">
                                    <v-text-field v-model="description" label="Description" style="padding-top: 1rem"  variant="outlined" required :rules="[rules.required]"></v-text-field>
                                </v-col>
...
<script setup>
        ...
        addChecker.value ?.validate().then(async ({
            ...
            if (isValid) {
                ...
                try {
                    ...
                    await axios.post(`divisions`,{params: {
                    id: id.value,
    ...
    const loaditems = async () => {
        ...
        await axios.get(`/api/loaddivisions`)
        .then(response => {
    ...
    if (result.isConfirmed) {
        ...
        //delete using axios
        await axios.delete('api/divisions/' + item.id);
